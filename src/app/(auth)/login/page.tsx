  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <Card className="w-full max-w-md border-slate-800 bg-slate-900">
        <CardHeader className="flex flex-col items-center justify-center text-center pb-6 space-y-2">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 overflow-hidden">
            {branding.logo_url ? (
              <img src={branding.logo_url} alt={branding.app_name} className="h-full w-full object-cover" />
            ) : (
              <span className="text-xl font-bold uppercase text-primary">
                {(branding.app_name || "C").charAt(0)}
              </span>
            )}
          </div>
