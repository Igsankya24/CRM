...
// POST /api/sales-registers/[id]/send-whatsapp
export async function POST(
  ...
  try {
    ...
    // Fetch the sales_register
    const { data: sales_register, error: qErr } = await supabase
    ...
    const phone = sales_register.mobile?.replace(/\D/g, "");
    const { data: contact } = await supabase
      .from("contacts")
      .select("id")
      .eq("account_id", profile.account_id)
      .eq("phone", sales_register.mobile)
      .maybeSingle();
    ...
    const messageText =
      `*Sales Register ${sales_register.sales_register_no}*\n\n` +
      `Dear ${sales_register.contact_person || sales_register.company_name},\n\n` +
      `Please find your sales register details:\n` +
      `📋 *Sales Register No:* ${sales_register.sales_register_no}\n` +
      `📅 *Date:* ${sales_register.entry_date}\n` +
      `🏢 *Company:* ${sales_register.company_name}\n` +
      `💰 *Grand Total:* ₹${Number(sales_register.grand_total).toLocaleString("en-IN", { minimumFractionDigits: 2 })}\n\n` +
      (pdfUrl ? `📄 *PDF:* ${pdfUrl}\n\n` : "") +
    ...
    await supabase.from("sales_register_logs").insert({
      ...
      channel: "whatsapp",
      recipient: sales_register.mobile,
    ...
    // Update sales_register status to 'sent' if it was draft
    ...
    console.error("[POST /api/sales-registers/[id]/send-whatsapp]", err);
