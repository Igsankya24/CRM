...
// POST /api/sales-registers/[id]/send-email
export async function POST(
  ...
  try {
    ...
    const { data: sales_register } = await supabase
      .from("sales-registers")
      .select("*")
      .eq("id", id)
      .is("deleted_at", null)
      .maybeSingle();
    if (!sales_register) return NextResponse.json({ error: "Sales Register not found" }, { status: 404 });
    ...
    const recipientEmail = body.email || sales_register.email;
    ...
    if (!resendKey) {
      ...
      await supabase.from("sales_register_logs").insert({
    ...
    const subject = `Sales Register ${sales_register.sales_register_no} from ${senderName}`;
...
<body style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto;">
  <div style="background: #1e293b; padding: 24px; border-radius: 8px 8px 0 0;">
    ...
    <p style="color: #94a3b8; margin: 4px 0 0;">Sales Register</p>
  </div>
  <div style="padding: 24px; background: #f8fafc; border-radius: 0 0 8px 8px;">
    <p>Dear ${sales_register.contact_person || sales_register.company_name},</p>
    <p>Thank you for your inquiry. Please find your sales register details below:</p>
    <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
      <tr style="background: #e2e8f0;">
        <td style="padding: 10px 14px; font-weight: bold;">Sales Register No</td>
        <td style="padding: 10px 14px;">${sales_register.sales_register_no}</td>
      </tr>
      <tr>
        <td style="padding: 10px 14px; font-weight: bold;">Date</td>
        <td style="padding: 10px 14px;">${sales_register.entry_date}</td>
      </tr>
      <tr style="background: #e2e8f0;">
        <td style="padding: 10px 14px; font-weight: bold;">Subject</td>
        <td style="padding: 10px 14px;">${sales_register.subject || "—"}</td>
      </tr>
      <tr>
        ...
        <td style="padding: 10px 14px; font-size: 18px; font-weight: bold; color: #0f4c81;">
          ₹${Number(sales_register.grand_total).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
        </td>
      </tr>
    </table>
    ${pdfUrl ? `<p><a href="${pdfUrl}" style="background: #0f4c81; color: #fff; padding: 10px 20px; border-radius: 6px; text-decoration: none; display: inline-block;">📄 Download PDF Sales Register</a></p>` : ""}
    ...
    if (!sendError && sales_register.status === "draft") {
      await supabase.from("sales-registers")
        .update({ status: "sent", updated_by: user.id })
        .eq("id", id);
      await supabase.from("sales_register_status_history").insert({
        sales_register_id: id, old_status: "draft", new_status: "sent",
    ...
    console.error("[POST /api/sales-registers/[id]/send-email]", err);
