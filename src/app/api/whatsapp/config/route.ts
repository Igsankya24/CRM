...
function supabaseAdmin(): SupabaseClient<Database> {
  if (!_adminClient) {
    ...
    _adminClient = createAdminClient<Database>(url, key)
  }
  return _adminClient as SupabaseClient<Database>
}
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.warn('Failed to fetch app name in whatsapp config api:', err);
    }
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            `This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one ${appName} user.`,
        },
        { status: 409 }
      )
    }
    ...
    try {
      ...
      encryptedVerifyToken = verify_token ? encrypt(verify_token) : undefined
      if (app_secret) {
    ...
    if (needsRegistration) {
      if (!pin) {
        return NextResponse.json(
          {
            error:
              `Two-step verification PIN is required to subscribe this number to ${appName}. ` +
    ...
    const baseRow: Partial<Database['public']['Tables']['whatsapp_config']['Update']> = {
      phone_number_id,
      waba_id: waba_id || null,
      access_token: encryptedAccessToken,
      verify_token: encryptedVerifyToken ?? undefined,
