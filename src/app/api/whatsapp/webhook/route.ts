...
// Shared admin Supabase client (merged from Whatsapp-Agent-main/src/lib/supabase.ts)
import { getAdminClient } from '@/lib/supabase/admin'
// AI agent auto-reply (merged from Whatsapp-Agent-main/src/lib/ai.ts)
import { getAIResponse } from '@/lib/ai/agent'
// Customer memory — per-contact AI fact store (migration 031)
...
export async function POST(request: Request) {
  ...
  // Resolve the App Secret. We find the config using the phone_number_id
  // or waba_id from the parsed body.
  ...
  try {
    ...
    let configRow: { app_secret?: string | null } | null = null
    if (phoneNumberId) {
    ...
    if (configRow?.app_secret) {
      appSecret = decrypt(configRow.app_secret)
    }
  } catch (err) {
...
async function processMessage(
  ...
  config: Database['public']['Tables']['whatsapp_config']['Row'],
  accessToken: string
) {
  ...
  // ============================================================
  // AI SALES AGENT AUTO-REPLY (upgraded from basic AI agent)
  //
  // If the conversation has ai_mode=true AND the inbound message
  // is a text message, the AI Sales Agent generates a knowledge-
  // enriched reply, extracts facts, scores the lead, and detects
  // handoff triggers.
  //
  // Fire-and-forget: AI errors must not block webhook 200 response.
  // Gated on contentText to skip media/location/reaction messages.
  // ============================================================
  if (config.ai_enabled === true && conversation.ai_mode === true && contentText && message.type === 'text') {
    void (async () => {
      try {
        ...
        if (config.openrouter_api_key) {
          try {
            ...
            console.error('[webhook] Failed to decrypt OpenRouter API key:', err)
          }
        }

        // Use the Sales Agent for knowledge-enriched replies
        const { generateSalesReply } = await import('@/lib/ai/sales-agent')
        const agentResult = await generateSalesReply({
          ...
          aiConfig: {
            ...
            systemPrompt: config.ai_system_prompt ?? undefined,
        ...
        // Send AI reply via Meta Cloud API using the configuration's credentials
        ...
        // Store AI reply in the messages table
        await supabaseAdmin().from('messages').insert({
          conversation_id: conversation.id,
          sender_type: 'bot',
          content_type: 'text',
          content_text: agentResult.reply,
          status: 'sent',
        })

        // Update conversation last_message_text with AI reply
        await supabaseAdmin()
          .from('conversations')
          .update({
            last_message_text: agentResult.reply,
            last_message_at: new Date().toISOString(),
            unread_count: 0, // AI replied — reset unread counter
            updated_at: new Date().toISOString(),
          })
          .eq('id', conversation.id)

        // Handle handoff if triggered
        if (agentResult.shouldHandoff) {
          const { executeHandoff } = await import('@/lib/ai/handoff')
          await executeHandoff({
            ...
            contactPhone: contactRecord.phone,
            contactName: contactRecord.name,
            reason: agentResult.handoffReason || 'AI-detected handoff trigger',
            whatsappConfig: {
        ...
        console.error('[webhook] AI sales agent auto-reply failed:', aiErr)
      }
    })()
  }
