...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? `our ${branding.app_name || 'CRM with AI'} account`,
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? `our ${branding.app_name || 'CRM with AI'} account`;
    const message = `Join ${accountName} on ${branding.app_name || 'CRM with AI'} using this link (valid for ${result?.expiresInDays} days): ${url}`;
