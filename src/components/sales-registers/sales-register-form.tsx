...
import type { SalesRegister, SalesRegisterItem, QuotationTaxType, CompanyBankAccount } from "@/types";
...
interface SalesRegisterFormProps {
  initial?: SalesRegister | null;
  prefill?: Partial<{
    company_name: string;
...
// ── Main Component ────────────────────────────────────────────
export function SalesRegisterForm({ initial, prefill, mode }: SalesRegisterFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    transportation_charges: String(initial?.transportation_charges ?? 0),
    packing_charges:        String(initial?.packing_charges ?? 0),
    other_charges:          String(initial?.other_charges ?? 0),
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select(
        "company_name,bank_account_name,bank_account_type,bank_account_number,bank_name,bank_ifsc,terms_and_conditions,gst_number,address,phone,email,manager_name,manager_designation,signature_url,sales_register_terms_text"
      )
      ...
      .then(({ data }) => {
        if (data) {
          ...
          // Fetch terms directly from company settings as requested (no quotation fallback)
          // ONLY populate if we are creating a new document (no initial)
          if (!initial) {
            if (data.sales_register_terms_text) {
              setTermsText(data.sales_register_terms_text);
            } else {
              setTermsText("");
            }
          }
  ...
  const { basicTotal, taxAmount } = calcTotals(
    parsedItems,
    form.tax_type,
    parseFloat(form.custom_tax_rate) || null
  );

  const transportationVal = parseFloat(form.transportation_charges) || 0;
  const packingVal        = parseFloat(form.packing_charges) || 0;
  const otherVal          = parseFloat(form.other_charges) || 0;

  const grandTotal = Math.round((basicTotal + taxAmount + transportationVal + packingVal + otherVal) * 100) / 100;
    ...
    async (statusOverride?: string) => {
      ...
      try {
        const payload = {
          ...
          transportation_charges: parseFloat(form.transportation_charges) || 0,
          packing_charges:        parseFloat(form.packing_charges)        || 0,
        ...
        const url   = mode === "create" ? "/api/sales-registers" : `/api/sales-registers/${initial!.id}`;
        ...
        toast.success(mode === "create" ? "Sales Register created!" : "Sales Register updated!");
        router.push(`/sales-registers/${data.sales_register.id}`);
      } catch (err) {
    ...
    <div className="space-y-4 pb-24">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Field label="Subject">
            <input
              ...
              placeholder="Sales Register for industrial valves"
              className={inputCls}
            />
          </Field>
        </div>
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="grid gap-3 sm:grid-cols-3 pt-2">
              <Field label="Transportation (₹)">
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, transportation_charges: e.target.value }))}
                  placeholder="0.00"
                  min="0"
                  step="0.01"
                  className={inputCls}
              ...
              <Field label="Packing (₹)">
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, packing_charges: e.target.value }))}
                  placeholder="0.00"
                  min="0"
                  step="0.01"
                  className={inputCls}
          ...
          <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-4 space-y-3">
            ...
            {parseFloat(form.transportation_charges) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Transportation</span>
                <span className="font-semibold text-white">{formatINR(parseFloat(form.transportation_charges))}</span>
              </div>
            )}
            {parseFloat(form.packing_charges) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Packing Charges</span>
                <span className="font-semibold text-white">{formatINR(parseFloat(form.packing_charges))}</span>
              </div>
            )}
            {parseFloat(form.other_charges) > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Other Charges</span>
                <span className="font-semibold text-white">{formatINR(parseFloat(form.other_charges))}</span>
              </div>
            )}
        ...
        <div className="pt-4 border-t border-slate-800">
          <Field label="Customer Notes / Internal Remarks">
            <textarea
              value={form.notes}
              onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
              placeholder="Provide any additional comments or instructions for the customer..."
              rows={3}
              className={inputCls}
            />
          </Field>
      ...
      >
        <p className="text-xs text-slate-500">
          Edit manager details below. These values are saved to Company Settings and printed on the sales_register signature block.
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
