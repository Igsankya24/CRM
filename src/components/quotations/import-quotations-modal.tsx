...
interface ImportQuotationsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
}

export function ImportQuotationsModal({ open, onOpenChange, onImported }: ImportQuotationsModalProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (json.length === 0) { toast.error('No rows found in the selected file.'); return; }
      setParsedRows(json);
  ...
  async function handleImport() {
    ...
    try {
      // Fetch default quotation terms from company settings
      const { data: companySettings } = await supabase
        .from('company_settings')
        .select('quotation_terms_text, manager_name, manager_designation')
        .eq('account_id', accountId)
        .maybeSingle();

      const defaultTerms = companySettings?.quotation_terms_text || '';
      const managerName = companySettings?.manager_name || '';
      const managerDesignation = companySettings?.manager_designation || '';
      ...
      for (const row of parsedRows) {
        ...
        const quotationNoInput = getValue(['quotationno', 'quotno', 'number', 'no']);
        const entryDateInput = getValue(['entrydate', 'date', 'quotationdate']);
        const contactPerson = getValue(['contactperson', 'contact', 'name', 'contactname']);
        const email = getValue(['email', 'emailid', 'mail']);
        const mobile = getValue(['mobile', 'phone', 'contactnumber', 'mobileno']);
        const altMobile = getValue(['altmobile', 'alternatephone', 'altphone']);
        ...
        const subject = getValue(['subject', 'description', 'particulars']);
        ...
        const status = ['draft', 'sent', 'accepted', 'rejected', 'expired'].includes(rawStatus.toLowerCase())
          ? rawStatus.toLowerCase()
          : 'draft';

        // Generate unique quotation number if not provided
        let quotationNo = quotationNoInput;
        if (!quotationNo) {
          const { data: nextNo } = await supabase.rpc('next_quotation_no', { p_account_id: accountId });
        ...
        if (qErr) {
          console.error('Error inserting quotation:', qErr.message);
          failed++;
          continue;
        }

        // Insert terms snapshot
        await supabase.from('quotation_terms').insert({
    ...
    setImporting(false);
    if (imported > 0) {
      toast.success(`Import completed! ${imported} quotation${imported > 1 ? 's' : ''} imported.`);
      onImported();
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="bg-slate-900 border-slate-700 text-slate-200 sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-white">Import Quotations</DialogTitle>
          <DialogDescription className="text-slate-400">
            Upload an Excel (.xlsx, .xls) or CSV (.csv) file. Required column: <span className="font-semibold text-slate-300">Company Name</span>.
            Optional: Date, Quotation No, Contact Person, Mobile, Email, Address, State, GST No, Subject, Grand Total, Status.
          </DialogDescription>
        </DialogHeader>
        ...
        {!result ? (
          <div className="space-y-4">
            <div
              className="border-2 border-dashed border-slate-700 rounded-lg p-6 text-center hover:bg-slate-800/50 cursor-pointer transition-colors"
            ...
            >
              ...
              <p className="text-[10px] text-slate-500 mt-1">Accepts .xlsx, .xls and .csv files</p>
            </div>
