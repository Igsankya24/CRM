...
interface ImportProformasModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported: () => void;
}

export function ImportProformasModal({ open, onOpenChange, onImported }: ImportProformasModalProps) {
  ...
  async function handleImport() {
    ...
    try {
      // Fetch default proforma settings from company settings
      const { data: companySettings } = await supabase
        .from('company_settings')
        .select('proforma_terms_text, manager_name, manager_designation, bank_account_name, bank_account_type, bank_account_number, bank_name, bank_ifsc')
        .eq('account_id', accountId)
        .maybeSingle();

      const defaultTerms = companySettings?.proforma_terms_text || '';
      ...
      for (const row of parsedRows) {
        ...
        const proformaNoInput = getValue(['proformano', 'pfno', 'number', 'no']);
        const entryDateInput = getValue(['entrydate', 'date', 'proformadate']);
        ...
        const status = ['draft', 'sent', 'paid', 'cancelled'].includes(rawStatus.toLowerCase())
          ? rawStatus.toLowerCase()
          : 'draft';

        // Generate unique proforma number if not provided
        let proformaNo = proformaNoInput;
        if (!proformaNo) {
          const { data: nextNo } = await supabase.rpc('next_proforma_no', { p_account_id: accountId });
        ...
        if (pErr) {
          console.error('Error inserting proforma:', pErr.message);
          failed++;
          continue;
        }

        // Insert terms snapshot
        await supabase.from('proforma_terms').insert({
    ...
    setImporting(false);
    if (imported > 0) {
      toast.success(`Import completed! ${imported} proforma${imported > 1 ? 's' : ''} imported.`);
      onImported();
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="bg-slate-900 border-slate-700 text-slate-200 sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-white">Import Proformas</DialogTitle>
          <DialogDescription className="text-slate-400">
            ...
            Optional: Date, Proforma No, Contact Person, Mobile, Email, Address, State, GST No, Subject, Grand Total, Status.
          </DialogDescription>
        </DialogHeader>
