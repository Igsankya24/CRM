...
import {
  ...
  MoreHorizontal, Loader2, CheckCircle2, XCircle, RefreshCw,
} from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
...
import { ProformaPDFButton } from "./proforma-pdf";
...
interface ProformaActionsProps {
  ...
  onStatusChange?: (id: string, status: ProformaStatus) => void;
}
...
export function ProformaActions({
  ...
  const handleSendWhatsApp = async () => {
    const phone = proforma.mobile?.trim();
    if (!phone) {
    ...
    router.push(
      `/inbox?phone=${encodeURIComponent(phone)}&name=${encodeURIComponent(
        proforma.contact_person || proforma.company_name || ""
      )}&docType=proforma&docId=${proforma.id}&docNo=${encodeURIComponent(proforma.proforma_no)}`
    );
  };
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch(`/api/proformas/${proforma.id}/send-email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: proforma.email }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Email send failed");
      toast.success("Email sent!");
      onStatusChange?.(proforma.id, "issued");
    } catch (err) {
  ...
  const handleStatusChange = async (status: ProformaStatus) => {
    try {
      ...
      onStatusChange?.(proforma.id, status);
    } catch (err) {
    ...
    return (
      <>
        <div className="flex items-center gap-1">
          ...
          <button
            title="Edit"
            onClick={() => router.push(`/proformas/${proforma.id}/edit`)}
            ...
            <DropdownMenuContent align="end">
              ...
              {proforma.status !== "issued" && (
                <DropdownMenuItem onClick={() => handleStatusChange("issued")}>
                  <CheckCircle2 className="h-4 w-4 text-blue-400" /> Mark Issued
                </DropdownMenuItem>
              )}
              {proforma.status !== "waiting_payment" && (
                <DropdownMenuItem onClick={() => handleStatusChange("waiting_payment")}>
                  <CheckCircle2 className="h-4 w-4 text-amber-400" /> Mark Waiting Payment
                </DropdownMenuItem>
              )}
              {proforma.status !== "paid" && (
                <DropdownMenuItem onClick={() => handleStatusChange("paid")}>
                  <CheckCircle2 className="h-4 w-4 text-emerald-400" /> Mark Paid
                </DropdownMenuItem>
              )}
              {proforma.status !== "cancelled" && (
                <DropdownMenuItem onClick={() => handleStatusChange("cancelled")}>
                  <XCircle className="h-4 w-4 text-red-400" /> Mark Cancelled
                </DropdownMenuItem>
              )}
              <DropdownMenuSeparator />
              <DropdownMenuItem
        ...
        <Dialog open={confirmDelete} onOpenChange={(v) => !v && setConfirmDelete(false)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                This will permanently delete proforma{" "}
                <strong>{proforma.proforma_no}</strong> for{" "}
                <strong>{proforma.company_name}</strong>. This cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <button
        ...
        <WhatsAppValidationDialog
          ...
          onEditCustomer={() => router.push(`/proformas/${proforma.id}/edit`)}
        />
      </>
    );
  }
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {!proforma.sales_register && (
          <button
            ...
            disabled={converting}
            className={`${btnBase} bg-violet-600/20 border border-violet-500/30 text-violet-300 hover:bg-violet-600/30 disabled:opacity-50`}
          >
      ...
      <Dialog open={confirmDelete} onOpenChange={(v) => !v && setConfirmDelete(false)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Proforma</DialogTitle>
            <DialogDescription>
              Permanently delete <strong>{proforma.proforma_no}</strong>? This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
