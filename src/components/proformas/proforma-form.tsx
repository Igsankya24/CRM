...
// ── Main Component ────────────────────────────────────────────
export function ProformaForm({ initial, prefill, mode }: ProformaFormProps) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select(
        "company_name,bank_account_name,bank_account_type,bank_account_number,bank_name,bank_ifsc,terms_and_conditions,gst_number,address,phone,email,manager_name,manager_designation,signature_url,proforma_terms_text"
      )
      ...
      .then(({ data }) => {
        if (data) {
          ...
          if (!initial) {
            if (data.proforma_terms_text) {
              setTermsText(data.proforma_terms_text);
            } else {
              setTermsText("");
            }
          }
  ...
  const handleSave = useCallback(
    async (statusOverride?: string) => {
      ...
      try {
        ...
        const url   = mode === "create" ? "/api/proformas" : `/api/proformas/${initial!.id}`;
        ...
        toast.success(mode === "create" ? "Proforma created!" : "Proforma updated!");
    ...
    <div className="space-y-4 pb-24">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Field label="Subject">
            <input
              ...
              placeholder="Proforma for industrial valves"
              className={inputCls}
            />
          </Field>
        </div>
      ...
      >
        <p className="text-xs text-slate-500">
          Edit manager details below. These values are saved to Company Settings and printed on the proforma signature block.
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
